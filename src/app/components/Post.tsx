  ...
  const [isCommentModalOpen, setIsCommentModalOpen] = useState(false);
    ...
    <div className="w-fit max-auto mb-6 p-4 border rounded shadow-sm">
      ...
      <div className="flex items-center mt-4">
        ...
        />
        <button className="ml-4 flex items-center" onClick={() => setIsCommentModalOpen(true)}>
          <FiMessageSquare className="w-6 h-6 text-gray-500" />
          <span className="ml-1">{post.comments?.length || 0}</span>
        </button>
      </div>
      <CommentModal post={post} currentUserId={currentUserId} isOpen={isCommentModalOpen} onRequestClose={() => setIsCommentModalOpen(false)} />
    </div>
  );
};
